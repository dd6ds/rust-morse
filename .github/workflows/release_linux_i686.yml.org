  ...
  build:
    ...
    - name: Install MinGW and zip
      run: |
        ...
        sudo dpkg --add-architecture i386 && sudo apt-get update && sudo apt-get install libssl-dev:i386 clang pkg-config libx11-dev libxext-dev libxft-dev libxinerama-dev libxcursor-dev libxrender-dev libxfixes-dev libpango1.0-dev libgl1-mesa-dev libglu1-mesa-dev cmake gettext librust-alsa-dev librust-udev-dev libfltk1.3-dev libfltk1.3t64 mingw-w64 gcc-multilib curl g++ pkg-config libx11-dev libasound2-dev libudev-dev librust-alsa-sys-dev libstd-rust-dev:i386 libasound2-dev
    ...
    - name: Delete existing asset
      run: |
        ...
        ASSET_ID=$(gh api \
          repos/${{ github.repository }}/releases/tags/${{ github.ref_name }} \
          | jq '.assets[] | select(.name == "main.zip") | .id')
        if [ -n "$ASSET_ID" ]; then
          gh api --method DELETE \
            /repos/${{ github.repository }}/releases/assets/$ASSET_ID
        fi
        ...
        ASSET_ID=$(gh api \
          repos/${{ github.repository }}/releases/tags/${{ github.ref_name }} \
          | jq '.assets[] | select(.name == "main.tar.gz") | .id')
        if [ -n "$ASSET_ID" ]; then
          gh api --method DELETE \
            /repos/${{ github.repository }}/releases/assets/$ASSET_ID
        fi
        ...
        ASSET_ID=$(gh api \
          repos/${{ github.repository }}/releases/tags/${{ github.ref_name }} \
          | jq '.assets[] | select(.name == "morse_windows-i686-v${{ steps.get_version.outputs.cargo_version }}.zip ") | .id')
        if [ -n "$ASSET_ID" ]; then
          gh api --method DELETE \
            /repos/${{ github.repository }}/releases/assets/$ASSET_ID
        fi
    ...
    - name: Upload Release Asset
      ...
      with:
        ...
        asset_path: release/morse_windows-i686-v${{ steps.get_version.outputs.cargo_version }}.zip
        asset_name: morse_windows-i686-v${{ steps.get_version.outputs.cargo_version }}.zip
